...
llm_config_autogen = {
    "seed": 42,  # Установите начальное значение для генерации случайных чисел
    "temperature": 0.7,  # Температура для управления креативностью вывода
    "config_list": [
        {
            "model": "sberbank-ai/rugpt3large",  # Модель на русском языке
            "tokenizer": "sberbank-ai/rugpt3large",  # Токенизатор для модели
        },
        {
            "model": "DeepPavlov/rubert-base-cased",  # Еще одна модель на русском языке
            "tokenizer": "DeepPavlov/rubert-base-cased",
        },
{
            "model": "blinoff/gpt2-russian",  # Еще одна модель на русском языке
            "tokenizer": "blinoff/gpt2-russian",
        },
    ],
    "timeout": 60000,  # Таймаут для запросов к модели (в миллисекундах)
}
...
def get_models():
    models = {}
    for config in llm_config_autogen["config_list"]:
        model_name = config["model"]
        models[model_name] = load_model_and_tokenizer(model_name)
    return models

# Загрузка моделей
# loaded_models = get_models()

# Пример генерации текста с одной из моделей
...
model, tokenizer = loaded_models["sberbank-ai/rugpt3large"]
output_text = generate_text(model, tokenizer, "Какой интересный день!")
print(output_text)
